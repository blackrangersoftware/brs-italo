...
def setup():
    ...
    os.chdir('inputs')
    if not os.path.isdir('italo'):
        subprocess.check_call(['git', 'clone', args.url, 'italo'])
    os.chdir('..')
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/italo/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/italocoin-project/italo', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/italo', exist_ok=True)
    os.chdir('builder/inputs/italo')
    if args.pull:
