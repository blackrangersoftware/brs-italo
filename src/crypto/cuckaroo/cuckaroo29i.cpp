...
#include "int-util.h"
#include "cuckaroo29i.h"


void Cuckaroo29I::setsipkeys(const unsigned char *keybuf,siphash_keys *keys) {
...
void Cuckaroo29I::setheader(const unsigned char *header, const uint32_t headerlen, siphash_keys *keys) {
...
uint64_t  Cuckaroo29I::rotl(uint64_t x, uint64_t b) {
	return (x << b) | (x >> (64 - b));
}
void Cuckaroo29I::sip_round(void) {
...
void  Cuckaroo29I::hash24(const uint64_t nonce) {
	v3 ^= nonce;
	sip_round(); sip_round();
	v0 ^= nonce;
	v2 ^= 0xff;
...
uint64_t Cuckaroo29I::xor_lanes(void) {
	return (v0 ^ v1) ^ (v2  ^ v3);
}
uint64_t Cuckaroo29I::sipblock(siphash_keys *keys, const uint32_t edge,uint64_t  *buf) {
	v0=keys->k0;
	v1=keys->k1;
	v2=keys->k2;
	v3=keys->k3;
...
int Cuckaroo29I::verify(uint32_t edges[PROOFSIZE], siphash_keys *keys) {
...
int Cuckaroo29I::hash(const void* in, size_t len, uint32_t nonce, uint32_t *edges,void* out)
{
	...
	if (retval == 0)
	{
		...
		blake2b_state S[1];
		blake2b_init( S, 32 );
		blake2b_update( S, ( const uint8_t * )hashdata, 174 );
